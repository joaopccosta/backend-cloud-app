...
build:
	make -C app setup_python test build VERSION=$(VERSION) ECR_REGISTRY=$(ECR_REGISTRY) VERSION=$(VERSION)

push:
	make -C app setup_python test build ecr-login ecr-push VERSION=$(VERSION) ECR_REGISTRY=$(ECR_REGISTRY) VERSION=$(VERSION)
